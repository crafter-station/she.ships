...
export default function EditGate({ badge }: EditGateProps) {
  ...
  const verify = useCallback(async (emailToVerify: string) => {
    ...
    try {
      ...
      if (res.ok) {
        sessionStorage.setItem("badge_email", emailToVerify);
        setAuthorized(true);
      } else {
        sessionStorage.removeItem("badge_email");
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("badge_email");
    if (stored) {
      verify(stored);
    } else {
      setChecking(false);
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-xs flex flex-col gap-4">
        <div>
          ...
          <p className="mt-1 text-white/40 text-sm">
            Enter the email you registered with on Luma to edit this badge.
          </p>
        </div>
        <input
          type="email"
